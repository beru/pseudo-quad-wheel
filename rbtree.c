...
static node
lookup_node (rbtree t, void *key, rb_compare_func compare, void *userdata)
{
  node n = t->root;
  while (n != NULL)
    {
      ...
      if (comp_result == 0)
	{
	  if (global_lookup_helper)
	    global_lookup_helper (n->key, userdata);
	  return n;
	}
...
void *
rbtree_lookup (rbtree t, void *key, void *userdata)
{
 ...
 */
int
rbtree_insert (void *ps, rbtree t, void *key, void *value)
{
  node inserted_node = new_node (ps, key, value, RED, NULL, NULL);
  if (t->root == NULL)
    {
      ...
      while (1)
	{
	  ...
	  if (comp_result == 0)
		...
		{
		  global_vreplace_func (ps, n->value, value);
		  ret = 0;
		}

	      global_vfree_func (ps, key, value);
	      global_kfree_func (ps, key);
...
void
rbtree_delete (void *ps, rbtree t, void *key)
{
  node child;
  ...
  global_vfree_func (ps, keytodelete, valuetodelete);
  global_kfree_func (ps, keytodelete);
  mpool_free (n, node_pool);
}

int
walk_tree_helper (void *ps, rbtree_node n, void *userdata,
		  rb_walk_callback callback)
{
  if (n == NULL)
    return 0;

  if (n->right != NULL)
    walk_tree_helper (ps, n->right, userdata, callback);
  if (callback (ps, n->key, n->value, userdata))
    return -1;
  if (n->left != NULL)
    walk_tree_helper (ps, n->left, userdata, callback);

  return 0;
}

int
rbtree_walk (void *ps, rbtree t, void *userdata, rb_walk_callback callback)
{
  return walk_tree_helper (ps, t->root, userdata, callback);
}

void
destroy_node (void *ps, rbtree_node n)
{
  if (n == NULL)
    return;
  if (n->right != NULL)
    destroy_node (ps, n->right);
  if (n->left != NULL)
    destroy_node (ps, n->left);
  global_vfree_func (ps, n->key, n->value);
  global_kfree_func (ps, n->key);
  mpool_free (n, node_pool);
}

void
rbtree_destroy (void *ps, rbtree t)
{
  destroy_node (ps, t->root);
  mpool_free (t, rbtree_pool);
}

void
rbtree_module_init (void *ps,
...
{
  rbtree_pool = mpool_create (ps, sizeof (struct rbtree_t));
  node_pool = mpool_create (ps, sizeof (struct rbtree_node_t));
