...
#undef sp
	...
	ipp = &(ps->ec.callstack[ps->ec.callstacksp].ip);
	*ipp = ip;
	ps->ec.callstacksp++;
		...
		switch(ip->op) {
			...
			case OP_PUSHVAR: {
				...
				if (n->context_id == context_id) {
					...
					if (!v) v = scope_chain_object_lookup(ps,scope, (ObjKey *)varname);
			...
			case OP_PUSHFUN: {
				...
				fun_prototype = value_object_utils_new_object(ps);
			...
			case OP_ADD: {
				...
				if (TOP.vt == VT_STRING || TOQ.vt == VT_STRING) {
					...
					v = unistrcat(ps,TOQ.d.str, TOP.d.str);
			...
			case OP_SHF: {
				...
				value_toint32(ps,&TOP);
				t1 = (int)TOQ.d.num;
				t2 = ((unsigned int)TOP.d.num) & 0x1f;
			...
			case OP_DEC: {
				...
				Value *v;
				if (TOP.vt != VT_VARIABLE) evaldie("operand not left value\n");
				v = TOP.d.lval;
				value_tonumber(ps,v);
			...
			case OP_NEWFCALL: {
				...
				tocall_index = sp - stackargc - 1;
				tocall = &stack[tocall_index];
				...
				if (as_constructor) {						/* new Constructor */
					...
					proto = value_object_lookup(tocall, (ObjKey *)PROTOTYPE.unistr, NULL);
			...
			case OP_EVAL: {
				...
				if (stackargc > 0) {
					if (stack[sp - stackargc].vt == VT_UNDEF) evaldie("eval undefined value\n");
			...
			case OP_DELETE: {
				...
				if (count == 1) {
					Value *v;
					if (TOP.vt != VT_VARIABLE) evaldie("delete a right value\n");
					v = TOP.d.lval;
			...
			case OP_OBJECT: {
				...
				obj = object_make(ps, &stack[sp-itemcount*2], itemcount*2);
			...
			case OP_ARRAY: {
				...
				Object *obj;
				topevaln(itemcount);
				obj = object_make_array(ps, &stack[sp-itemcount], itemcount);
				pop_n(itemcount - 1);
				value_erase(TOP);
				value_make_object(TOP, obj);
				break;
			...
			case OP_EFINAL: {
				...
				last_op = trylist->d.td.last_op;
				tojmp = (last_op == LOP_JMP ? trylist->d.td.ld.tojmp : 0);
			...
			case OP_WITH: {
				...
				n = trylist_new(ps,TL_WITH, scope, currentScope);
