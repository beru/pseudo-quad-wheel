...
ObjKey* objkey_new(void* ps, const unichar* strkey, int flag)
{
...
ObjKey* objkey_new2(void* ps, const char* strkey, int flag)
{
...
ObjKey* objkey_dup(void* ps, const ObjKey* ori)
{
...
static
int objkey_compare(void* left, void* right)
{
	ObjKey* a = left;
	ObjKey* b = right;
...
static
void objkey_free(void* ps, void* data)
{
	// printf("Free key: "); 
	//   _uniprint(data);
...
void iterobj_free(PState* ps, IterObj* iobj)
{
...
static
void iterobj_insert(void* ps, IterObj* io, ObjKey* key)
{
	if (io->count >= io->size) {
		io->size += 20;
...
FuncObj* funcobj_new(PState* ps, Func* func)
{
...
void funcobj_free(PState* ps, FuncObj* fobj)
{
	if (fobj->scope) {
...
void object_free(PState* ps, Object* obj)
{
	// printf("Free obj: %x\n", (int)obj);
	switch (obj->ot) {
	case OT_STRING:
...
Object* object_make(PState* ps, Value* items, int count)
{
...
Object* object_make_array(PState* ps, Value* items, int count)
{
	Object* obj = object_new(ps);
	UNISTR (12) unibuf;
...
Value* value_dup(PState* ps, Value* v)
{
...
void value_free(PState* ps, void* data)
{
	Value* v = data;
	value_erase(*v);
...
void value_toprimitive(PState* ps, Value* v)
{
	if (v->vt == VT_OBJECT) {
		Value res;
		Object* obj = v->d.obj;
...
void value_tostring(PState* ps, Value* v)
{
...
void value_tonumber(PState* ps, Value* v)
{
	double a = 0;
	switch (v->vt) {
	case VT_BOOL:
...
void value_toint32(PState* ps, Value* v)
{
	double d = 0.0;
	value_tonumber(ps, v);
...
void value_toobject(PState* ps, Value* v)
{
	if (v->vt == VT_OBJECT) {
		return;
	}
...
void object_insert(PState* ps, Object* obj, ObjKey* key, Value* value)
{
...
void object_try_extern(PState* ps, Object* obj, int inserted_index)
{
...
Value* value_object_key_assign(PState* ps, Value* target, Value* key, Value* value, int flag)
{
	int arrayindex = -1;
...
void value_object_delete(PState* ps, Value* target, Value* key)
{
	if (target->vt != VT_OBJECT) {
		return;
	}
...
void value_subscript(PState* ps, Value* target, Value* key, Value* ret, int right_val)
{
	if (!target) {
...
static
int _object_getkeys_callback(void* ps, void* key, void* value, void* userdata)
{
	ObjKey* ok = key;
	int flag = KEYFLAG(ok);
...
static
void _object_getkeys(PState* ps, Value* target, IterObj* iterobj)
{
	if (!target) {
		return;
	}
...
void value_object_getkeys(PState* ps, Value* target, Value* ret)
{
...
ScopeChain* scope_chain_new(PState* ps, int cnt)
{
...
Value* scope_chain_object_lookup(PState* ps, ScopeChain* sc, ObjKey* key)
{
	Value* ret;
...
ScopeChain* scope_chain_dup_next(PState* ps, ScopeChain* sc, Value* next)
{
	if (!sc) {
...
void scope_chain_free(PState* ps, ScopeChain* sc)
{
...
void object_set_length(PState* ps, Object* obj, int len)
{
	int flag = 0;
...
Value* value_object_utils_new_object(PState* ps)
{
	Value* n = value_new(ps);
...
void object_utils_insert(PState* ps, Object* obj, const unichar* key, Value* val, int deletable, int writable, int emuable)
{
	int flag = 0;
	if (!deletable) {
		flag |= OM_DONTDEL;
...
void object_utils_insert2(PState* ps, Object* obj, const char* key, Value* val, int deletable, int writable, int emuable)
{
	int flag = 0;
	if (!deletable) {
		flag |= OM_DONTDEL;
...
void value_object_utils_insert(PState* ps, Value* target, const unichar* key, Value* val, int deletable, int writable, int emuable)
{
	if (target->vt != VT_OBJECT) {
		warn("Target is not object\n");
		return;
...
void value_object_utils_insert2(PState* ps, Value* target, const char* key, Value* val, int deletable, int writable, int emuable)
{
	if (target->vt != VT_OBJECT) {
		warn("Target is not object\n");
		return;
...
void object_utils_insert_array(PState* ps, Object* obj, int key, Value* val, int deletable, int writable, int emuable)
{
	int flag = 0;
	UNISTR (12) unibuf;
	if (!deletable) {
...
void value_object_utils_insert_array(PState* ps, Value* target, int key, Value* val, int deletable, int writable, int emuable)
{
	if (target->vt != VT_OBJECT) {
		warn("Target is not object\n");
		return;
...
udid userdata_register(PState* ps, UserDataReg* udreg)
{
...
UserData* userdata_new(PState* ps, udid id, void* data)
{
...
void userdata_free(PState* ps, UserData* ud)
{
	udid id = ud->id;
...
void userdata_set(PState* ps, Object* obj, udid id, void* data)
{
	if (obj->ot != OT_OBJECT) {
...
void* userdata_get(PState* ps, Object* obj, udid id)
{
...
static
void objvalue_free(void* ps, void* key, void* value)
{
...
static
void objvalue_vreplace(void* ps, void* target, void* from)
{
	Value* tv = target;
	Value* fv = from;
	value_erase(*tv);
...
static
void objvalue_lookup_helper(void* key, void* userdata)
{
	if (!userdata) {
		return;
	}
...
static
int objvalue_insert_helper(void* key)
{
	int flag = KEYFLAG (key);
