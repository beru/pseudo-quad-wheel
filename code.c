...
OpCodes* codes_new(PState* ps, int size)
{
...
static
int codes_insert(PState* ps, OpCodes* c, Eopcode code, void* extra)
{
...
OpCodes* codes_join(PState* ps, OpCodes* a, OpCodes* b)
{
...
OpCodes* codes_join3(PState* ps, OpCodes* a, OpCodes* b, OpCodes* c)
{
	return codes_join(ps, codes_join (ps, a, b), c);
}

OpCodes* codes_join4(PState* ps, OpCodes* a, OpCodes* b, OpCodes* c, OpCodes* d)
{
...
OpCodes* code_push_undef(PState* ps)
{
	NEW_CODES(OP_PUSHUND, 0);
}

OpCodes* code_push_bool(PState* ps, int v)
{
	NEW_CODES(OP_PUSHBOO, v);
}

OpCodes* code_push_num(PState* ps, double* v)
{
	NEW_CODES(OP_PUSHNUM, v);
}

OpCodes* code_push_string(PState* ps, const unichar* str)
{
	NEW_CODES(OP_PUSHSTR, str);
}

OpCodes* code_push_index(PState* ps, unichar* varname)
{
...
OpCodes* code_push_this(PState* ps)
{
	NEW_CODES(OP_PUSHTHS, 0);
}

OpCodes* code_push_top(PState* ps)
{
	NEW_CODES(OP_PUSHTOP, 0);
}

OpCodes* code_push_top2(PState* ps)
{
	NEW_CODES(OP_PUSHTOP2, 0);
}

OpCodes* code_unref(PState* ps)
{
	NEW_CODES(OP_UNREF, 0);
}

OpCodes* code_push_args(PState* ps)
{
	NEW_CODES(OP_PUSHARG, 0);
}

OpCodes* code_push_func(PState* ps, Func* fun)
{
	NEW_CODES(OP_PUSHFUN, fun);
}

OpCodes* code_push_regex(PState* ps, regex_t* reg)
{
	NEW_CODES(OP_PUSHREG, reg);
}

OpCodes* code_local(PState* ps, const unichar* varname)
{
	NEW_CODES(OP_LOCAL, varname);
}

OpCodes* code_nop(PState* ps)
{
	NEW_CODES(OP_NOP, 0);
}

OpCodes* code_neg(PState* ps)
{
	NEW_CODES(OP_NEG, 0);
}

OpCodes* code_pos(PState* ps)
{
	NEW_CODES(OP_POS, 0);
}

OpCodes* code_bnot(PState* ps)
{
	NEW_CODES(OP_BNOT, 0);
}

OpCodes* code_not(PState* ps)
{
	NEW_CODES(OP_NOT, 0);
}

OpCodes* code_mul(PState* ps)
{
	NEW_CODES(OP_MUL, 0);
}

OpCodes* code_div(PState* ps)
{
	NEW_CODES(OP_DIV, 0);
}

OpCodes* code_mod(PState* ps)
{
	NEW_CODES(OP_MOD, 0);
}

OpCodes* code_add(PState* ps)
{
	NEW_CODES(OP_ADD, 0);
}

OpCodes* code_sub(PState* ps)
{
	NEW_CODES(OP_SUB, 0);
}

OpCodes* code_less(PState* ps)
{
	NEW_CODES(OP_LESS, 0);
}

OpCodes* code_greater(PState* ps)
{
	NEW_CODES(OP_GREATER, 0);
}

OpCodes* code_lessequ(PState* ps)
{
	NEW_CODES(OP_LESSEQU, 0);
}

OpCodes* code_greaterequ(PState* ps)
{
	NEW_CODES(OP_GREATEREQU, 0);
}

OpCodes* code_equal(PState* ps)
{
	NEW_CODES(OP_EQUAL, 0);
}

OpCodes* code_notequal(PState* ps)
{
	NEW_CODES(OP_NOTEQUAL, 0);
}

OpCodes* code_eequ(PState* ps)
{
	NEW_CODES(OP_STRICTEQU, 0);
}

OpCodes* code_nneq(PState* ps)
{
	NEW_CODES(OP_STRICTNEQ, 0);
}

OpCodes* code_band(PState* ps)
{
	NEW_CODES(OP_BAND, 0);
}

OpCodes* code_bor(PState* ps)
{
	NEW_CODES(OP_BOR, 0);
}

OpCodes* code_bxor(PState* ps)
{
	NEW_CODES(OP_BXOR, 0);
}

OpCodes* code_shf(PState* ps, int right)
{
	NEW_CODES(OP_SHF, right);
}

OpCodes* code_assign(PState* ps, int h)
{
	NEW_CODES(OP_ASSIGN, h);
}

OpCodes* code_subscript(PState* ps, int right_val)
{
	NEW_CODES(OP_SUBSCRIPT, right_val);
}

OpCodes* code_inc(PState* ps, int e)
{
	NEW_CODES(OP_INC, e);
}

OpCodes* code_dec(PState* ps, int e)
{
	NEW_CODES(OP_DEC, e);
}

OpCodes* code_fcall(PState* ps, int argc)
{
	NEW_CODES(OP_FCALL, argc);
}

OpCodes* code_newfcall(PState* ps, int argc)
{
	NEW_CODES(OP_NEWFCALL, argc);
}

OpCodes* code_ret(PState* ps, int n)
{
	NEW_CODES(OP_RET, n);
}

OpCodes* code_delete(PState* ps, int n)
{
	NEW_CODES(OP_DELETE, n);
}

OpCodes* code_chthis(PState* ps, int n)
{
	NEW_CODES(OP_CHTHIS, n);
}

OpCodes* code_pop(PState* ps, int n)
{
	NEW_CODES(OP_POP, n);
}

OpCodes* code_jfalse(PState* ps, int off)
{
	NEW_CODES(OP_JFALSE, off);
}

OpCodes* code_jtrue(PState* ps, int off)
{
	NEW_CODES(OP_JTRUE, off);
}

OpCodes* code_jfalse_np(PState* ps, int off)
{
	NEW_CODES(OP_JFALSE_NP, off);
}

OpCodes* code_jtrue_np(PState* ps, int off)
{
	NEW_CODES(OP_JTRUE_NP, off);
}

OpCodes* code_jmp(PState* ps, int off)
{
	NEW_CODES(OP_JMP, off);
}

OpCodes* code_object(PState* ps, int c)
{
	NEW_CODES(OP_OBJECT, c);
}

OpCodes* code_array(PState* ps, int c)
{
	NEW_CODES(OP_ARRAY, c);
}

OpCodes* code_key(PState* ps)
{
	NEW_CODES(OP_KEY, 0);
}

OpCodes* code_next(PState* ps)
{
	NEW_CODES(OP_NEXT, 0);
}

OpCodes* code_eval(PState* ps, int argc)
{
	NEW_CODES(OP_EVAL, argc);
}

OpCodes* code_stry(PState* ps, int trylen, int catchlen, int finlen)
{
...
OpCodes* code_etry(PState* ps)
{
	NEW_CODES(OP_ETRY, 0);
}

OpCodes* code_scatch(PState* ps, const unichar* var)
{
	NEW_CODES(OP_SCATCH, var);
}

OpCodes* code_ecatch(PState* ps)
{
	NEW_CODES(OP_ECATCH, 0);
}

OpCodes* code_sfinal(PState* ps)
{
	NEW_CODES(OP_SFINAL, 0);
}

OpCodes* code_efinal(PState* ps)
{
	NEW_CODES(OP_EFINAL, 0);
}

OpCodes* code_throw(PState* ps)
{
	NEW_CODES(OP_THROW, 0);
}

OpCodes* code_with(PState* ps, int withlen)
{
	NEW_CODES(OP_WITH, withlen);
}

OpCodes* code_ewith(PState* ps)
{
	NEW_CODES(OP_EWITH, 0);
}

OpCodes* code_typeof(PState* ps)
{
	NEW_CODES(OP_TYPEOF, 0);
}

OpCodes* code_debug(PState* ps)
{
	NEW_CODES(OP_DEBUG, 0);
}

OpCodes* code_reserved(PState* ps, int type, unichar* id)
{
...
JmpPopInfo* jpinfo_new(PState* ps, int off, int topop)
{
...
void code_reserved_replace(PState* ps, OpCodes* ops, int step_len, int break_only, const unichar* desire_label, int topop)
{
	for (int i=0; i<ops->code_len; ++i) {
		...
		if (ri->type == RES_CONTINUE) {
			...
			psfree (ri);
			if (topop) {
				ops->codes[i].data = jpinfo_new(ps, step_len + ops->code_len - i, topop);
				ops->codes[i].op = OP_JMPPOP;
			}else {
...
void code_decode(PState* ps, OpCode* op, int currentip)
{
...
void codes_free(PState* ps, OpCodes* ops)
{
	// TODO
	psfree(ops->codes);
	psfree(ops);
...
void codes_print(PState* ps, OpCodes* ops)
{
	int i = 0;
