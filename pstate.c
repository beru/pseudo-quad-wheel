...
{
  if (((PSTATE *) ps)->ec.memcontext == 0) return free(p);
...
pstate_svc (void *ps, int i, char *s)
{
  ...
  if (((PSTATE *)ps)->ec.jmpbufsp <= 0) abort();
